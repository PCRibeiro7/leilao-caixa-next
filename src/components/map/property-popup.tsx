...
export default function PropertyPopup({ caixaId }: PropertyPopupProps) {
    const [property, setProperty] = useState<GeocodedProperty | null>(null);
    ...
    const imageSrc = retryCount > MAX_RETRIES ? FALLBACK_IMAGE : getImageSrc(caixaId, retryCount);

    return (
        <Popup className="custom-popup" minWidth={260} maxWidth={260} eventHandlers={{ add: fetchProperty }}>
            {loading || !property ? (
                <div className="popup-card popup-card--loading">
                    <div className="popup-skeleton__img" />
                    <div className="popup-skeleton__body">
                        <div className="popup-skeleton__line popup-skeleton__line--title" />
                        <div className="popup-skeleton__line popup-skeleton__line--subtitle" />
                        <div className="popup-skeleton__line popup-skeleton__line--price" />
                        <div className="popup-skeleton__line popup-skeleton__line--details" />
                    </div>
                </div>
            ) : (
