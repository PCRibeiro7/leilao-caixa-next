...
async function fetchGoogleMapsGeocodeData(
    ...
    try {
        ...
        if (response.data.results.length > 0) {
            ...
            return buildGeocodedProperty(property, latitude, longitude, precision, GeocodeProvider.GoogleMaps);
        }
...
async function fetchNominatinGeocodeData(
    ...
    const address = formatAddress(property, attemptCount);
    try {
        const viewboxCandidate = mapStateAndCityToBoundingBox.get(property.state)?.get(property.city);
        ...
        if (response.data.length > 0) {
            ...
            const precision = mapAttemptCountToPrecision[attemptCount];
            return buildGeocodedProperty(property, latitude, longitude, precision, GeocodeProvider.Nominatim);
        }
...
async function fetchRadarGeocodeData(
    ...
    try {
        ...
        if (response.data.addresses.length > 0) {
            ...
            const precision = mapAttemptCountToPrecision[attemptCount];
            return buildGeocodedProperty(property, latitude, longitude, precision, GeocodeProvider.Radar);
        }
...
async function fetchGeocodeMapsGeocodeData(
    ...
    try {
        ...
        if (response.data.length > 0) {
            ...
            const longitude = parseFloat(location.lon);
            const precision = mapAttemptCountToPrecision[attemptCount];
            return buildGeocodedProperty(property, latitude, longitude, precision, GeocodeProvider.GeocodeMaps);
        }
